    ...
    <tbody>
        @foreach($data as $el)
            <tr>
                ...
                <td class="text-center font-{{$el['font-style']}}" {!! $el['border-top']?'style="border-top:2px solid black;"':''!!} title="{{($el['status-info']->getUnfilteredNumber()-$el['status-info']->getNumber())?'+ '.($el['status-info']->getUnfilteredNumber()-$el['status-info']->getNumber()).' hidden marks':''}}">{{$el['status-info']->getNumber()}}</td>
