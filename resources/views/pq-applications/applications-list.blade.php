@foreach($prequalify_request_objs as $prequalify_request_obj)
    ...
    @endphp
    <li class="list-group-item pq-application pt-1 pb-1" data-status="{{$current_status}}" data-id="{{$prequalify_request_obj->id}}">
        <div class="d-table">
            <div class="d-table-row">
                <div class="d-table-cell align-middle">{!! $lp_div !!}</div>
                <div class="d-table-cell align-middle p-1 pr-2" style="line-height: 1;"><span style="font-size: 11px;">{!! \DateTime::createFromFormat('Y-m-d H:i:s',$prequalify_request_obj->created_at)->format('M j, Y \<\b\r\/\>\@ H:i') !!}</span></div>
                <div class="d-table-cell align-middle">
                    {!! $poo_icon !!}
                    {!! $clone_icon !!}
                    {!! ($prequalify_request_obj->quiz_at?'<i class="fas fa-money-bill-alt text-success"></i>':'') !!}
                    {{$prequalify_request_obj->tmfSubject->first_name}} {{$prequalify_request_obj->tmfSubject->last_name}}
                    {!! $phone_icon !!}
                </div>
            </div>
        </div>
