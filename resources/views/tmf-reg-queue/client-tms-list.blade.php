    ...
    @php
        ...
        foreach($data_val as $el){
                if ($tmf_reg_queue_status->deadline_warning_hours > 0 && $tmf_reg_queue_status->deadline_overdue_hours > 0) {
                    $index='badge-success';
                    if (($el['pending_in_this_status_delta'] > $tmf_reg_queue_status->deadline_warning_hours * 3600) &&
                        ($el['pending_in_this_status_delta'] < $tmf_reg_queue_status->deadline_overdue_hours * 3600))
                        $index = 'badge-warning';
                    elseif ($el['pending_in_this_status_delta'] > $tmf_reg_queue_status->deadline_overdue_hours * 3600)
                        $index = 'badge-danger';
                    $result[$index]++;
                }
        }
