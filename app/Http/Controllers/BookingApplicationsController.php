...
use App\Tmoffer;
use App\traits\BookingApplicationsTrait;
use Carbon\Carbon;
...
class BookingApplicationsController extends Controller
{
    use SaveToSent;
    use BookingApplicationsTrait;
...
//        dd($data);
        ...
        $prequalify_request_objs=PrequalifyRequest::all();
        $show_lead_statuses=[];
        $show_stats=1;
...
//                    $prequalify_request_obj->approved_for_booking_at=$now;
                    $prequalify_request_email_type_id=3;
                    $sms_message=sprintf('Congratulations, %s! Your request for a free call with a Trademark Factory advisor has been approved. Check your email for more details.',
                        $prequalify_request_obj->tmfSubject->first_name);
...
//                ->bcc([$andrei->Email])
            ...
            if(in_array($request->action,['approve-for-booking','approved-and-booked']))
...
//            $prequalify_request->notes=$request->notes;
            ...
            $prequalify_request_lead_statuses_log->lead_status_id=$request->lead_status;
