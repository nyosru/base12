...
class TmfRegQueueStatusMaintainerController extends Controller
{
    /**
    ...
    public function index(){
        ...
        return view('tmf-reg-queue-status-maintainer.index',
            compact('flag_settings','flags_prefixes','queue_status_type_objs'));
    }

    public function loadRootStatuses(){
        $tmf_reg_queue_root_status_objs=TmfRegQueueRootStatus::orderBy('place_id','asc')->get();
        $status_type='root-status';
        return view('tmf-reg-queue-status-maintainer.statuses-list',
            compact('tmf_reg_queue_root_status_objs','status_type'));
    }

    public function loadRootStatusOptions(){
        $tmf_reg_queue_root_status_objs=TmfRegQueueRootStatus::orderBy('place_id','asc')->get();
        return view('tmf-reg-queue-status-maintainer.root-status-options',
            compact('tmf_reg_queue_root_status_objs'));
    }
    ...
    public function dashboardTssOptions(Request $request){
        ...
        $cipostatus_status_formalized_tss_objs=DashboardTssTemplate::where('cipostatus_status_formalized_id',
            ...
            ->get();
        return view('tmf-reg-queue-status-maintainer.dashboard-tss-options',
            compact('global_status_tss_objs','cipostatus_status_formalized_tss_objs'));
    }

    public function loadSubStatuses(Request $request){
        $tmf_reg_queue_root_status_obj=TmfRegQueueRootStatus::find($request->id);
        if($tmf_reg_queue_root_status_obj){
            $status_type='sub-status';
            return view('tmf-reg-queue-status-maintainer.sub-statuses-list',
                compact('tmf_reg_queue_root_status_obj','status_type'));
    ...
    private function saveNewRootStatus(Request $request){
        if($request->id){
            $obj=TmfRegQueueRootStatus::find($request->id);
            $obj->name = $request->name;
        }else {
            $obj = new TmfRegQueueRootStatus();
            $obj->name = $request->name;
            $obj->place_id = TmfRegQueueRootStatus::all()->count();
        }
        $obj->save();
        return 'Done';
    }
    ...
    private function saveNewSubStatus(Request $request){
        if($request->id){
            $obj=TmfRegQueueStatus::find($request->id);
        }else{
            $obj=new TmfRegQueueStatus();
            $obj->place_id=TmfRegQueueStatus::all()->count();
        }
        $obj->name=$request->name;
        $obj->tmf_reg_queue_root_status_id=$request->parent_id;
    ...
    public function reorderRootStatuses(Request $request){
        ...
        if(json_last_error()==JSON_ERROR_NONE){
            foreach ($ids as $index=>$id){
                $obj=TmfRegQueueRootStatus::find($id);
