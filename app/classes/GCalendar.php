...
$file='/json/client_secret_111699364449-9gnk14hat8oj8fb57s2ucjuetklp69rt.apps.googleusercontent.com.json';
if(!defined('APPLICATION_NAME'))
    define('APPLICATION_NAME', 'Google Calendar API PHP Quickstart');
if(!defined('CLIENT_SECRET_PATH'))
    define('CLIENT_SECRET_PATH', app('config')->get('filesystems')['disks']['local']['root'].$file);
...
class GCalendar
{
    ...
    private function access($accessToken,$tmfsales_info=null){
        ...
        if ($this->client->isAccessTokenExpired()) {
            ...
            if(!is_null($tmfsales_info)){
                $tmfsales_info->google_calendar_access=json_encode($this->accessToken);
                $tmfsales_info->save();
            }
            $this->client->setAccessToken($this->accessToken);
        }
    ...
    public function authenticateByTMFSales($tmfsales_info){
//        var_dump(json_decode($tmfsales_info['google_calendar_access'],true));
        return $this->access(json_decode($tmfsales_info->google_calendar_access,true));
    }
...
//                echo "id:".$calendarListEntry->getID().' summary:'.$calendarListEntry->getSummary().' timezone: '.$calendarListEntry->getTimeZone().'<br/>';
                $calendar_data[]=[
    ...
    public function insertEvent($calendarId='primary',$text,$description,$datetime,$length=45){
//        file_put_contents(__DIR__.'/../test.txt',$datetime);
    ...
    public function insertEventWithoutReminders($calendarId='primary',$text,$description,$datetime,$length=45){
        $event = new \Google_Service_Calendar_Event([
            'summary' => $text,
            'location' => '',
            'description' => $description,
...
//                echo "<pre>".print_r($event->start,true)."</pre>";
    ...
    public function getEventObjByID($event_id){
        $event = $this->service->events->get('primary', $event_id);
        return $event;
    }

    public function getEventShortDataByID($event_id){
        $event = $this->service->events->get('primary', $event_id);
        if($event->start->dateTime)
            $start = $event->start->dateTime;
        else
