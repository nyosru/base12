...
use App\classes\rftth\TemplateTranslator;
use App\Mail\OutreachEmail1Sent;
use App\RfTthEmailsSeq;
use App\TmfBooking;
...
//        dd($objs);
        ...
        foreach ($objs as $obj){
            ...
            $tmf_subject_contact=TmfSubjectContact::where('tmf_subject_id',$tmf_subject->id)
                ...
                ->first();
//            dd($obj);
            if($obj->rfTthEmail->code!='Drip') {
                $data = [
                    'subj' => $obj->rfTthEmail->subj,
...
//            ->cc($andrei->Email, $andrei->FirstName . ' ' . $andrei->LastName)
    ...
    private function getData($date){
        $result=[];
        $objs=RfTthEmailsSeq::where('scheduled_at','like','%'.$date->format('Y-m-d H:i').'%')
            ->where('rftth_email_id','!=',1)
            ->get();
//        dd($objs);
        foreach ($objs as $obj){
